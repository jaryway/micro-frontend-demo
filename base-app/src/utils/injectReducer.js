...
function injectReducer(reducers) {
  return (Component, { forwardedRef = false } = {}) => {
    return () => (
      <ReactReduxContext.Consumer>
        {({ store }) => {
          console.log('injectReducer-com', Component);
          ...
          const Connect = function({ forwardedRef, ...rest }) {
            return <Component ref={forwardedRef} {...rest} />;
          };

          const WrappedComponent = forwardRef((props, ref) => {
            console.log('WrappedComponent', ref);
            return <Connect {...props} ref={el => console.log(el)} forwardedRef={ref} />;
          });
