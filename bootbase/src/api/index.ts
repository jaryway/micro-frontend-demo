...
export const request4useRequest = (service: any) => {
  ...
  console.log('request4useRequest', service);
  if (typeof service !== 'string') {
    const { url, ...options } = service;
    return request(url, options).then((resp) => resp.data.data);
  }
  return request(service).then((resp) => resp.data.data);
...
export function getTokenInfoFromLocalStorage() {
  ...
  const cookieToken = document.cookie.split('; ').filter((m) => m.startsWith('token='))[0];
...
function saveTokenToLocalStorage(token: any) {
  ...
  document.cookie = `token=${expires_in}; path=/`;
...
function refreshTokenAsync(refresh_token: string) {
  ...
  return normalApi('/uaa/api/oauth/token', options)
    .then((resp: any) => {
      const { data } = resp;
      console.log('refreshTokenAsync-resp', resp);
